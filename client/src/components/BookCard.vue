<template>
  <div class="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
    <div class="card mb-3">
      <a :href="`/book-detail_${book._id}`" class="card-img">
        <img :src="book.img" class="card-img-top" alt="book" />
        <div class="quick-add">
          <a
            data-bs-toggle="modal"
            :data-bs-target="`#qa`+book._id"
            >Quick add</a
          >
        </div>        
        <div
            class="modal fade"
            :id="`qa`+book._id"
            tabindex="-1"
            aria-hidden="true"
            @click.stop
          >
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                  <button
                    type="button"
                    class="btn-close"
                    data-bs-dismiss="modal"
                    aria-label="Close"
                    @click.prevent
                  ></button>
                </div>
                <div class="modal-body">
                  <div class="quick-add_box">
                    <img :src="book.img" onerror="this.src='data:image/png;base64,'" class="card-img-top" alt="book" />
                    <div class="quick-add_des">
                      <h3>{{ sstring(book.book_name, 95) }}</h3>
                      <p>{{ book.author }}</p>
                      <p>{{ book.type }}</p>
                      <div>
                        <span class="new-price">${{ book.price }}</span>
                        <span class="old-price">${{ book.price * 1.3}}</span>
                      </div>
                      <div class="mt-3">
                        {{ sstring(book.brief_des, 150) }}
                      </div>
                      <div class="mt-3">
                        <button class="btn btn-primary" @click.prevent="addBookToCart" v-if="isToken">Thêm vào giỏ hàng</button>
                        <button class="btn btn-outline-dark mt-2" @click.stop.prevent="login" data-bs-dismiss="modal" aria-label="Close" v-else>Đăng nhập để mua ngay</button>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="modal-footer pt-4">
                  <button class="btn btn-success mx-auto w-100 view-detail" 
                    data-bs-dismiss="modal"
                    aria-label="Close">
                    <a :href="`/book-detail_${book._id}`">Xem chi tiết sách >></a>
                  </button>
                </div>
            </div>
          </div>
        </div>
      </a>
      <div class="card-body">
        <a :href="`/book-detail_${book._id}`" class="card-title">{{ book.book_name }}</a>
        <p class="card-text">{{ sstring(book.type, 14) }}</p>
        <p class="card-text">
          <small class="text-muted">by: {{ sstring(book.author, 13) }}</small>
        </p>
      </div>
    </div>
  </div>
</template>

<script>
import CartAPI from '../api/cart'
import UserAPI from '../api/user'

export default {
  name: "BookCard",
  props: ["book"],
  data() {
    return {
      isToken: false
    }
  },
  methods: {
    async addBookToCart() {      
      const cart = await CartAPI.fetchItemById(this.book._id)
      if(!cart) {
        const data = {
          book: this.book._id,
          quantity: 1
        }
        await CartAPI.addItemToCart(data)
      } else {
          alert('Sách đã có trong giỏ!!!')
      }
    },
    sstring(str, length) {
      let substr = str.substring(0, length)
      if(str.length > length) {
        substr += '...'
      }
      return substr
    },
    login() {
      this.$router.push({name: 'Login', params: {id: this.book._id}})
    }
  },
  async created() {
    if(localStorage.getItem('bookshop')) {
      const data = await UserAPI.rememberUser()
      if(data.accessToken) {
        this.isToken = true
      }
    }
  },
};
</script>
